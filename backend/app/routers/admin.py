...
def _role_default_username(role: Role) -> str:
    """重导出 role_service.default_username（保留旧名兼容本文件内调用）。"""
    return role_service.default_username(role)


def _ensure_role_default_user(session: Session, role: Role) -> User:
    """重导出 role_service.ensure_default_user。"""
    return role_service.ensure_default_user(session, role)


def _batch_load_org_lookup(session: Session, roles: list[Role]) -> dict:
    """重导出 role_service.batch_load_org_lookup。"""
    return role_service.batch_load_org_lookup(session, roles)


def _role_to_read(role: Role, lookup: dict) -> RoleRead:
    """重导出 role_service.to_role_read。"""
    return role_service.to_role_read(role, lookup)


def _ensure_role_name_unique(
    session: Session, name: str, department_id: int | None, exclude_role_id: int | None = None
) -> None:
    """重导出 role_service.ensure_name_unique。"""
    return role_service.ensure_name_unique(
        session, name, department_id, exclude_role_id
    )
...
def _normalize_role_classification(
    ...
    """重导出 role_service.normalize_classification。"""
    return role_service.normalize_classification(
        session, segment_id, entity_id, department_id
    )


def _ensure_function_tag(session: Session, function_tag_id: int | None) -> None:
    """重导出 role_service.ensure_function_tag。"""
    return role_service.ensure_function_tag(session, function_tag_id)
